...
CORPUS = [
    ...
    ("Kristian dislikes em dashes in any prose, including commit messages.",
     ["Kristian"]),
    ("Reports to Kristian lead with the outcome and keep the evidence below it.",
     ["Kristian"]),
    ("The encrypted vault must never be copied; two backups reached GitHub that way.",
     ["vault", "GitHub"]),
    ("Neon Husky publishes tracks on Jamendo and short videos on TikTok.",
     ["Neon Husky", "Jamendo", "TikTok"]),
    ("Ledger reconciles the bank CSV export every month and flags anomalies.",
     ["Ledger"]),
    ("Free-tier Supabase pauses after seven idle days and drops out of DNS.",
     ["Supabase", "arvidsson.tech"]),
    ("The papers page on arvidsson.tech is served by a Cloudflare Worker.",
     ["arvidsson.tech", "Cloudflare"]),
...
GOLDEN = [
    ...
    ("how should I write to him?", "em dashes"),
    ("where does the music go?", "Jamendo"),
    ("who handles the invoices?", "bank CSV"),
    ("why did the papers page break?", "Supabase"),
    ("what serves arvidsson.tech?", "Cloudflare Worker"),
