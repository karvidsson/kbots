...
- Overlay → `config.yaml`, `agents.yaml`, `team.json`, agent CLAUDE.md files, service units
...
4. **No agent identities.** A CLAUDE.md that defines a named agent — its personality, permissions, and standing instructions — is overlay material; Core carries nothing beyond example templates.
...
├── agents/
│   └── rescue/                — example agent template (CLAUDE.md.example)
├── config/
...
Every agent points at a `project_dir` holding three things: CLAUDE.md for identity and tool documentation, `.mcp.json` for the MCP server config, and `.claude/settings.json` for permissions, env, and allowed tools.
...
Batch compression skips CLAUDE.md files: those are hand-tuned agent prompts, and every word in them is deliberate.
