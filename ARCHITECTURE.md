...
**Close-out is the user's verdict, not the owner's claim.** `goal_set status=done` posts the owner's summary as the last message in the room (goal, how it was reached, delivered / dropped / still-open tasks) with ✅/❌ seeded on it, stores the same text in `goals.summary`, and leaves the room writable. The connector resolves a reaction on that message through `goal_by_closing_message` (no status filter, unlike the kickoff lookup), admin-gated and elected to one client like the nomination cards. ✅ records the verdict (`record_verdict`), re-posts the summary once in `security.alert_channel` (fleet agents route with an empty `channels` list, so there is no per-agent home channel to put it in), drops the leftover tasks with reason "goal closed" (`drop_open_tasks`), and deletes the room; the goal row keeps its `channel_id` for history, and an *anchored* goal keeps its room, which it borrowed from its proposer and was never the goal's to remove. The order is worst-loss-last — verdict column, then the summary elsewhere, then the tasks, then the room — so a failure at any step leaves every step before it, and a delete that fails says so in the room it could not remove. ❌ records the verdict and moves the goal `done → executing`, the only exit from `done`; the store clears `closing_message_id` and the verdict on that transition so the next retirement posts again, and `goal_set` un-archives the room if it was ever made read-only. Open tasks are listed at retirement and dropped only on ✅, so a ❌ has nothing to un-drop; `goal_task action=carry target=<goal>` hands one to a successor goal before the verdict. `abandoned` keeps the older behaviour: closing notice, read-only room, no verdict. The janitor retires an expired proposal through the same close path: an anchored one gets the notice in the borrowed home channel; one with its own room loses the room only when a fetch of its history shows nothing but bot posts and `security.alert_channel` is set to carry the notice, otherwise it is closed and archived like any other retired room. A proposal room is routed, so a human may have asked something there, and an unattended tick must not delete that.

**Staffing is nominated, not assigned.** `goal_create` takes `participants` as `agent: why they are needed` pairs and refuses a bare agent id: the reason is enforced by the parser, so "every seat is justified" is a property of the feature rather than a habit the owner may drop. It also refuses more than `goals.max_participants` (default 4) rather than trimming, because which agent to drop is the owner's call to make and to be seen making. Each nominee gets its own message carrying ✅/❌, and the goal gets a kickoff card carrying ✅ and the owner's plan. Reactions resolve through `goal_nominations.message_id` in the connector's handler, admin-gated like HITL, and fall through to HITL when the message is not a goal card. A nomination is **not** participation: nothing is routed and no turns are spent until a human reacts ✅, and only that reaction calls `add_participant`. Declines are kept rather than deleted, so the record of who was considered and turned down survives. ✅ on the kickoff card does not advance the goal itself; it hands the owner a turn telling it to call `goal_set status=brainstorm`, so an anchored goal still acquires its channel through the one path that does that.

**Dynamic routing — no config edits:** participants of a live goal are routed into its channel straight from the goal store (`get_agent_for_channel` falls through to `goals.routed_participants_for_channel`), and they hear *every* message there (same semantics as `watch_channels`), with `NO_REPLY` as the abstain path. `goal_add_member` makes an agent live in the channel within ~10 s (TTL cache) once a human approves it — no restart. Mid-goal additions are HITL-gated rather than self-service: the requester must name what the new agent will do that nobody on the goal already can, and that reason is what the approvals channel shows. Each participating agent still needs its own bot account with guild access to the goals category (one Discord app per agent, as usual).
