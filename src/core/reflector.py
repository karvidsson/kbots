...
class Reflector:
    def __init__(self, agent_manager, config: dict | None = None,
        ...
        # with no entry reflects on its own default model rather than on a
        # name borrowed from a different vendor.
    ...
    def _model_for(self, llm) -> str | None:
        """Reflection model for this agent's provider — None means its default."""
        return self.models.get(getattr(llm, "name", ""))
    ...
    async def _extract_graph(self, agent_id: str) -> int:
        ...
        resp = await llm.complete(
            ...
            model=self._model_for(llm), timeout=180,
        )
        ...
        logger.info(f"Graph extraction: {agent_id} — {len(memories)} memories read, "
                    ...
                    f"(model={self._model_for(llm) or 'provider default'})")
        return linked
